...
function mostrarModal(tipo) {
    ...
    const modalTitle = document.getElementById('modal01Label');
    ...
    switch (tipo) {
        case 'opcoes':
            modalTitle.textContent = "Opções";
            sectionToShow = 'selectOpcoes'; // Nome da div a ser exibida
            break;
        case 'instrumento':
            modalTitle.textContent = "Instrumento";
            sectionToShow = 'selectInstrumento';
            break;
        case 'gravar':
            modalTitle.textContent = "Gravar Cifras";
            sectionToShow = 'modalGravar';

            // Lógica para exibir cifras ou textarea
            if (textoCifrasFrame.classList.contains('d-none') && textoCifras.contentDocument.body.innerHTML !== '') {
                escreverCifraTextArea.classList.remove('d-none');
                linksCifraClubList.classList.add('d-none');

                textoCifrasFrame.classList.remove('d-none'); // Mostrar temporariamente
                musicaAcordesTextArea.value = textoCifras.contentDocument.body.innerText;
                textoCifrasFrame.classList.add('d-none');    // Ocultar novamente

            } else {
                if (!textoCifras.classList.contains('d-none') && textoCifras.contentDocument.body.innerHTML !== '') {
                    textoCifras.classList.remove('d-none');
                    musicaAcordesTextArea.style.fontSize = '13px';
                    musicaAcordesTextArea.value = textoCifras.contentDocument.body.innerText;

                    if (formFile.value) {
                        textoCifras.classList.add('d-none');
                    }
                }
            }
            break;
        case 'salvar':
            // ... (lógica similar, adaptada para classes)
            break;
        // ... outros cases ...
    }
